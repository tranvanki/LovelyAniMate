...
# Custom static file server with CORS support for cross-origin requests
class CORSStaticFiles(StarletteStaticFiles):
    """
    Static file server that automatically injects CORS headers into all responses.
    Enables frontend and assets to be accessed from any origin without browser restrictions.
    """

    async def get_response(self, path: str, scope):
        response = await super().get_response(path, scope)

        # Inject CORS headers to allow requests from all origins
...
class AvatarStaticFiles(CORSStaticFiles):
    """
    Secure static file server for avatar images with format validation.
    Only serves image files with safe MIME types to prevent security exploits.
    """

    async def get_response(self, path: str, scope):
        # Whitelist of allowed image formats for avatar storage
