...
# Live2D model payload builder - prepares animation and expression data for the frontend
# Note: This class only constructs payload data; actual transmission is the caller's responsibility


class Live2dModel:
    """
    Manages Live2D model configuration and generates animation payloads.
    Loads model metadata, emotion mappings, and constructs WebSocket messages for the frontend.
    Does not handle network communication - only payload preparation.

    Attributes:
        model_dict_path: Path to JSON file containing all available Live2D model definitions
        live2d_model_name: Selected Live2D model identifier (e.g., 'mao_pro')
        model_info: Configuration dictionary for the selected model
        emo_map: Mapping of emotion keywords to Live2D animation parameters
        emo_str: Formatted string of available emotions for LLM context
    """
